...
int etapa2(int N)
{
    ...
    //Criar N work threads
    //Cada thread deverá partir de um genoma base unico, verificando
    //a existência de mutações.
    //No fim, cada thread imprime para um ficheiro as mutações resultantes:
    //    thread_id ...
    //Notificar a main thread que uma nova comparação foi terminada (usando memória partilhada)
    ...
    {
        //Criar N Threads
        pthread_create(&tid, NULL, compareGenomes, (void *)&tid);
    }
    ...
    while (g != NULL)
    {
        ...
        save_mutation_array(mutation_array, "result/mutations.txt", 0);
