...
typedef struct
{
    ...
    MUTATION_ARRAY *mutationArray;
} threadStruct, *ptr_threadStruct;
...
void main()
{
    ...
    //threadStruct arguments[N];
    //threadStruct *arguments = malloc(N * sizeof(threadStruct));
    threadStruct *arguments = (threadStruct *)calloc(N, sizeof(threadStruct));

    pthread_t tid[N];

    for (int i = 0; i < N; i++)
    {
        arguments[i].mutationArray = (MUTATION_ARRAY *)calloc(1, sizeof(MUTATION_ARRAY));
        arguments[i].genomeList = (GENOME_LIST *)calloc(1, sizeof(GENOME_LIST));
        arguments[i].genomeList = gl;
        arguments[i].i = i;
    }

    for (int i = 0; i < N; i++)
    {
        pthread_create(&tid[i], NULL, compareGenomes, &(arguments[i]));
    }
...
void *compareGenomes(void *arg)
{

    ptr_threadStruct trh = (ptr_threadStruct)arg;
    GENOME_LIST *gl = trh->genomeList;
    //read_genomes(gl, "input/cds.fna");
    MUTATION_ARRAY *mutation_array = trh->mutationArray;
    int i = trh->i;
    ...
    {
        char str[4];
        char path[] = "result/mutationsThread";
        char ext[] = ".txt";
        sprintf(str, "%d", i + 1);
        strcat(path, str);
        strcat(path, ext);
    ...
    printf("Thread %d completed! (%d genomes)\n", i + 1, count);
    pthread_exit(NULL);
}
